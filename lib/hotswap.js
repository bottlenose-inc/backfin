...
Hotswap.prototype._getPluginId = function(path, callback) {
  var folder = path.replace(/[^/]*$/,'');
  var self = this;
  fs.readdir(folder, function(err, list) {
    if(err) return callback(err);
    list.forEach(function(err, list){
      for(var i = 0; i < list.length;i++) {
        if(list[i] == 'mainfest.json') return callback(null, path);
      }
      self._getPluginId(folder);
    })
  });
}
...
Hotswap.prototype._watchFile = function(path) {
  ...
  var emit = _.debounce(function(){
    self._getPluginId(path , function(err, pluginId) { 
      self._emit('c' + path.replace('public/',''), ' ' + pluginId);
    })
  }, 400);
