...
function Hotswap(options) {
  ...
  this.getFiles(function(files){
    ...
    var manifests = files.filter(function(path){
      return path.indexOf('manifest.json') != -1;
    });
    var plugins = [];

    manifests.forEach(function(path) {
      var manifest = fs.readFileSync(path, 'utf-8');
      try {
        manifest = JSON.parse(manifest);
      } catch(e) {
        console.warn("Warning: Could not parse manifest.json file for "+path);
    ...
    var fd = fs.openSync(options.pluginDir + '/manifest.json', 'w+');
    fs.writeSync(fd, JSON.stringify({plugins: plugins}, null, 2));
    fs.closeSync(fd);
  });
}
...
Hotswap.prototype.getFiles = function(callback){
  ...
  this.paths.forEach(function(rootPath) {
...
Hotswap.prototype._watchFile = function(path, dir) {
  ...
  try {
    this._fsWatches.push(fs.watch(path, {persistent: true}, function (eventName, filename) {
      if (eventName == 'change') {
    ...
    if ((e.message || '').indexOf('EMFILE')) {
      console.warn('Please run "ulimit -n 2048" in your terminal');
    } else {
      throw e;
    }
  }
