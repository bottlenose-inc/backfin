...
BackfinCli.prototype.generateIndexCmd = function(callback) {
  var pluginIds = [];
  utils.walk(this.pluginDir, function(err, paths) {
    paths.forEach(function(path) {
      var relativePath = path.slice(this.pluginDir.length);
      var md = relativePath.match(/\/(.+)\/manifest\.json/)
      if(md && md[1]) { pluginIds.push(md[1]); }
    }.bind(this));

    var indexManifest = {plugins: pluginIds};
    var fd = fs.openSync(this.pluginDir + '/manifest.json', 'w+');
    fs.writeSync(fd, JSON.stringify(indexManifest, null, 2));
    fs.closeSync(fd);
    console.log("Found and indexed "+pluginIds.length+" plugin" + (pluginIds.length != 1 ? 's' : ''));
    callback();
  }.bind(this));
};
